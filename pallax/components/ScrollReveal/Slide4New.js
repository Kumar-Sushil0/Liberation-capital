...
export const ScrollRevealSectionSlide4New = ({ onAllColumnsVisible, currentSection, isScrollEnabled = true }) => {
  ...
  // Reset triggered columns when entering slide
  useEffect(() => {
    if (isInitialized && currentSection === 11) {
  ...
  // Reset to initial state when entering slide (currentSection === 11)
    ...
    const smoothScrollToStep = (step, skipAnimation = false) => {
      ...
      if (skipAnimation || step === 0) {
        ...
        if (step === 1) {
          window.dispatchEvent(new CustomEvent('subscrollComplete', { 
            detail: { section: 11, step: 1 } 
          }));
        }
        
        if (step === 2) {
          window.dispatchEvent(new CustomEvent('subscrollComplete', { 
            detail: { section: 11, step: 2 } 
          }));
        }
        
        if (step === 3) {
          window.dispatchEvent(new CustomEvent('subscrollComplete', { 
            detail: { section: 11, step: 3 } 
          }));
          if (onAllColumnsVisible) {
            onAllColumnsVisible();
          }
      ...
      const animateScroll = (currentTime) => {
        ...
        if (progress < 1) {
          ...
          if (step >= 4 && currentSection === 11) {
            setTimeout(() => {
              if (window.gotoNextSlide && currentSection === 11) {
      ...
      <div className={styles.slide4MobileContainer}>
        ...
        <div className={styles.greyLineContainerMobile}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <p className={styles.greyLineTextMobile} style={{ marginBottom: '10px' }}>
              This is self-evolution with teeth.
            </p>
            <p className={styles.greyLineTextMobile}>
              Where psychology sharpens and philosophy steadies,
              <br />
              play becomes the engine that reshapes who you&apos;ll be by Monday.
            </p>
          </div>
        </div>
        ...
        <div className={styles.scrollRevealMasterContainer} style={{
          ...
          <div className={styles.greyLineContainer}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <p className={styles.greyLineTextNoWrap}>
                Where psychology sharpens and philosophy steadies,</p>
                <p className={styles.greyLineTextNoWrap}>
                play becomes the engine that reshapes who you&apos;ll be by Monday.
              </p>
            </div>
