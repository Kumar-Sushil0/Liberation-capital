...
/**
 * Slide1New - New slide component inserted after reviews slide
 ...
 */
const Slide1New = React.forwardRef(({ onNextSlide }, ref) => {
  ...
  const handleBeginTransmission = () => {
    ...
    // Show main video and start playing
    setTimeout(() => {
      if (videoRef.current) {
  ...
  // Listen for pause events on the video to sync state
  useEffect(() => {
  ...
  return (
    <div ref={ref} className={styles.slide1Container}>
      ...
      <div className={styles.videoPlaceholder}>
        {/* Thumbnail image - shown initially, hidden when started */}
        <img
          src="https://lidbucketnew.s3.ap-south-1.amazonaws.com/landingpage/slideassets/slide2thumb.png"
          alt="Video Thumbnail"
      ...
      >
        {hasStarted ? 'Stop' : 'Watch Game Trailer'}
      </button>
    </div>
  );
});

Slide1New.displayName = 'Slide1New';
