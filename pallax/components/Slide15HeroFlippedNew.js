...
/**
 * Slide15HeroFlippedNew - Replica of Slide6HeroFlipped component
 * Displays the flipped hero layout with experimental game philosophy
 * @returns {JSX.Element} Hero flipped slide component
 */
const Slide15HeroFlippedNew = ({ onAllColumnsVisible, currentSection, isScrollEnabled = true }) => {
  ...
  useEffect(() => {
    ...
    if (currentSection === 18 && prevSection !== 18 && isScrollEnabled) {
      setSkipTransitions(true);
      setVisibleColumns(0);
      const container = containerRef.current;
      if (container) {
    ...
    const smoothScrollToStep = (step, skipAnimation = false) => {
      ...
      const animateScroll = (currentTime) => {
        ...
        if (progress < 1) {
          ...
          if (step >= 3 && currentSection === 18) {
            setTimeout(() => {
              if (window.gotoNextSlide && currentSection === 18) {
      ...
      <div className={styles.scrollRevealMasterContainer}>
        ...
        <div className={styles.greyLineContainer} style={{ zIndex: 10 }}>
          <p className={styles.greyLineText} style={{ color: '#888888', opacity: 1 }}>
            Visitor psychology: &quot;There&apos;s a whole world waiting behind this initiation.&quot; &quot;This is not a one-time workshop. This is a universe.&quot;
          </p>
        </div>
            ...
            <div className={styles.heroContentMobile}>
              <div className={styles.heroTextMobile}>
                <p className={styles.heroParagraphMobile}>
                  <strong>Game Multiverse - Saga of Selves</strong>
                </p>

                <p className={styles.heroParagraphMobile}>
                  One weekend lights the spark you chase,
                </p>

                <p className={styles.heroParagraphMobile}>
                  but deeper worlds wait once you leave that place.
                </p>

                <p className={styles.heroParagraphMobile}>
                  Seasons sculpt you, stories pull you,
                </p>

                <p className={styles.heroParagraphMobile}>
                  and each new realm mirrors a self more true.
                </p>

                <p className={styles.heroParagraphMobile}>
                  This universe grows as your identity does too,
                </p>

                <p className={styles.heroParagraphMobile} style={{ color: '#888888' }}>
                  every level a doorway designed for you.
                </p>
              </div>
            </div>
          </div>
...
export default Slide15HeroFlippedNew;
